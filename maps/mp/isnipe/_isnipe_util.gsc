...
/*
isnipe by HaiiZ (rewritten version)
credits to ApeX Clan
*/
...
/*--------Put dvars here for private match!------*/
	...
	setDvarIfUninitialized( "isnipe_serverIP", "Ts: ts.altermilkyway.net:10010" );
	...
	//server IP
	level.serverIP = GetDvarInt( "^2Detrua^1<3" );
...
onPlayerSpawned()
{
	...
	for(;;)
	{
		...
		if ( level.antiCamp )
			self thread AntiCamp(level.MaxCampTime, 0);	
		...
		self thread maps\mp\gametypes\_hud_message::hintMessage("^2Welcome ^1" +self.name);
		self thread maps\mp\gametypes\_hud_message::hintMessage("^5Good Luck ^0& ^5have Fun");
...
showAlive()
{
	...
	self.sap setPoint("TOPLEFT", "TOPLEFT", 34, 110);
	
	self.sap.hidewheninmenu = true;
	self.sap.foreground = false;
	...
	while( game["state"] == "playing" && game[ "state" ] != "postgame" && !isInKillcam() )  
	{
		...
		if (self.pers["team"] == game["attackers"])
			self.sap setText("^6" + alive_attackers+ " ^7vs ^5" +alive_defenders);      
		
		else if (self.pers["team"] == game["defenders"])
			self.sap setText("^6" + alive_defenders+ " ^7vs ^5" +alive_attackers);
...
doKnife()
{
	...
	for (i=0; i < 4; i++)
	{
		self iPrintLnBold("^2THROWING KNIFE ROUND");
		wait 1.5;
	}
...
self thread deleteondeath(self.topone);
	...
	for(;;)
	{
	 top = self getBestPlayer();
		if (top["KD"] != 0)
		{
			self.topplayerstitle setText("^1TOP ^3PLAYER");
		...
		for(;;)
		{
			...
			self notify("checked_travel");
				...
				if( ( self.travelled < longDistance) || distance( self.startpos, self.origin) < 120 ) {
					...
					for( i=6; i>0; i--) {
						self iPrintLnBold("^2Move ur ass or u ll be FUCKED!!" );
						wait 1;
